...
(function(){
	...
	var wot = {
		...
		cache: {
		       get: function(v) {
			       console.log("ARRGH!! wot.cache.set() was called from api.js // " + v);
		       },

		       set: function(v) {
			       console.log("ARRGH!! wot.cache.get() was called from api.js // " + v);
		       }
		}
	};
	...
	var api = {
		...
		call: function (apiname, options, params, onerror, onsuccess) {
			...
			try {
				...
				// TODO: find a way how to include jQuery $.expand here
				$.extend(params, {
				...
				$.ajax({
					...
					success: function (data, status) {
						logger.log("api.call.success: url = " + url + ", status = " +
							status + "\n");
		...
		processcookies: function (current) {
			...
			if (!this.isregistered() || !prefs.get("my_cookies")) {
				return null;
			}
...
//				}, onready);
		...
		query: function (target, onupdate) {
			...
			wot.cache.set(target, constants.cachestatus.busy);
			onupdate([ target ]);
				...
				$(elem).children().each(function () {
					...
					if (child) {
						...
						if (typeof(obj[name]) == "object" &&
							...
							/* shouldn't happen... */
							logger.log("api.parse: attribute / child collision\n");
						}
					}
				});
