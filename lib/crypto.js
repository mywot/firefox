...
(function(){
	...
	var getnonce = exports.getnonce = function(seed)
	{
		...
		var witness_id = (prefs.get("witness") || {}).id || ""; // ((witness || {}).id || ""
	...
	{
		try {
			var witness = prefs.get("witness") || {}; // ((witness || {}).id || ""

			if (data && nonce) {
				var key = (witness).key;              // TODO: FIXIT!

				if (key) {
					return btoa(bintostr(arc4.crypt(
	...
	{
		try {
			var key = (prefs.get("witness") || {}).key;                // TODO: FIXIT!
