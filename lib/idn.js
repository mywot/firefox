...
(function(){
	...
	exports.toascii = this.toascii = function(target) {
		try {
			if (/[\[\:]/.test(target)) {
			...
			target.toLowerCase().split(".").forEach(function(part) {
				if (/^[\w\-]*$/.test(part)) {
					...
					result.push(_this.ace + _this.punycode.encode(part));
				}
			});

			return result.join(".");
		} catch (e) {
			logger.fail("idn.toascii: failed with " + e);
		}

		return target;
	};

	exports.tounicode = this.tounicode = function(target) {
		try {
			if (!target) {
				return target;
			}
			...
			target.toLowerCase().split(".").forEach(function(part) {
				if (part.indexOf(_this.ace) == 0) {
					result.push(_this.punycode.decode(
						part.replace(_this.ace, "")));
				} else {
					result.push(part);
				}
			});
			...
			logger.fail("idn.tounicode: failed with " + e);
		}
	...
	this.punycode = function() {
		...
		this.encode_digit = function(d, flag) {
		...
		this.adapt = function(delta, numpoints, firsttime) {
			var k;

			delta = firsttime ? Math.floor(delta / damp) : (delta >> 1);
			delta += Math.floor(delta / numpoints);

			for (k = 0;  delta > (((base - tmin) * tmax) >> 1);
					k += base) {
				delta = Math.floor(delta / (base - tmin));
			}

			return Math.floor(k + (base - tmin + 1) * delta /
						(delta + skew));
		};
		...
		this.encode_basic = function(bcp, flag) {
			bcp -= (bcp - 97 < 26) << 5;
		...
		this.decode = function(input, preserveCase) {
			...
			for (ic = basic > 0 ? basic + 1 : 0; ic < input_length; ) {
				...
				out = output.length + 1;
				bias = _th.adapt(i - oldi, out, oldi === 0);
			...
			return _this.utf16.encode(output);
		};
		...
		this.encode = function(input, preserveCase)
		{
			...
			if (preserveCase) {
				/* Preserve case, step1 of 2: Get a list of the unaltered
					string */
				case_flags = _this.utf16.decode(input);
			}

			/* Converts the input in UTF-16 to Unicode */
			input = _this.utf16.decode(input.toLowerCase());
			...
			for (j = 0; j < input_length; ++j) {
				if (input[j] < 0x80) {
					output.push(String.fromCharCode(case_flags ?
						_th.encode_basic(input[j], case_flags[j]) : input[j]));
				}
			}
					...
					if (ijv == n) {
						...
						for (q = delta, k = base; ; k += base) {
							...
							output.push(String.fromCharCode(
								_th.encode_digit(t + (q - t) %
									(base - t), 0)));

							q = Math.floor((q - t) / (base - t));
						}

						output.push(String.fromCharCode(_th.encode_digit(q,
							preserveCase && case_flags[j] ? 1:0 )));

						bias = _th.adapt(delta, h + 1, h == b);
						delta = 0;
						++h;
					}
				}
