...
const apiutils = require("api-utils/utils/data"); // we are using base64Encode
...
(function(){
	...
	var getnonce = exports.getnonce = function(seed)
	{

		var witness_id = prefs.get("witness_id") || "";
		/* must be unique, not necessarily unpredictable */
		var data = "nonce:" + constants.version +
						//":" + window.navigator.language +     // TODO: FIXIT!
						":" + counter++ +
						":" + Date.now() +
						":" + Math.random() +
						":" + witness_id +
		...
		return bintohex(sha1.sha1str(data));
	};

	var encrypt = exports.encrypt = function(data, nonce)
	{
		logger.log("- crypto.encrypt / params = " + JSON.stringify(arguments));
		...
		try {
			...
			if (data && nonce) {
				...
				if (key) {
					return apiutils.base64Encode(bintostr(arc4.crypt(
							arc4.create(sha1.hmacsha1hex(key, nonce)),
							strtobin(data))));
				}
			}
		} catch (e) {
			logger.fail("crypto.encrypt: failed with ", e);
		}

		return null;
	};

	var authenticate = exports.authenticate = function(data) {
		try {
			var key = prefs.get("witness_key") || "";           // TODO: FIXIT!

			if (key) {
				return bintohex(sha1.hmacsha1hex(key, data));
			}
		} catch (e) {
			logger.fail("crypto.authenticate: failed with ", e);
		}
	...
	var sha1 = {
		hmacsha1hex: function(hexkey, str)
		{
			...
			var inner = this.sha1bin(ipad.concat(strtobin(str)));
			return this.sha1bin(opad.concat(inner));
		},

		sha1bin: function(bin)
		{
			return this.sha1str(bintostr(bin));
		},

		sha1hex: function(hex)
		{
			return this.sha1str(hextostr(hex));
		},
