	...
	exports.gethostname = this.gethostname = function(url_value) {
		...
		try {
			...
			if (_this.issupportedscheme(cleaned_url)) {
				var match = host.re.exec(cleaned_url);

				if (match && match[host.host]) {
					var host = idn.toascii(match[host.host]); // TODO: Fix it ASAP
					...
					if (!_this.isprivate(host)) {
						return _this.encodehostname(host, match[host.path]);
					}
				}
			}
		} catch (e) {
