...
//			});
	...
	var geticon = exports.geticon = function (data) {
		try {
			...
			logger.fail("core.geticon: failed with " + e);
		}
...
//			};
			...
			var ss = geticon(geticon(data), 19, prefs.get("accessible"));
			logger.log("*******   " + ss);
			//view.contentURL =
...
//				prefs.get("accessible"));
			...
			logger.fail("core.seticon: failed with " + e);
		}
	};
	...
	var updatetabwarning = exports.updatetabwarning = function (tab, data) {
		try {
			...
			preferences.forEach(function (item) {
				settings[item] = preferences.get(item);
			});
...
/*
	...
	*/
	...
	var run = exports.run = function (_widget) {
		...
		widget.panel.port.emit("wotMessaging", {
			message: "constants",
			constants: constants,
			prefs: prefs.getall()
		});
